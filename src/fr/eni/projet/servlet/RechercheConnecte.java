...
/**
 * Servlet implementation class RechercheConnecte
 */
@WebServlet("/rechercheCo")
public class RechercheConnecte extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RechercheConnecte() {
        super();
		...
		if(nomArticle.length() >= 1 && cat == 0) {
			...
			try {
				...
				if(listeArticleBySearch != null) {
						...
						this.getServletContext().getRequestDispatcher("/WEB-INF/jsp/indexConnecte.jsp").forward( request, response );
		...
		if(cat >= 1  && ((nomArticle == null) || (nomArticle.length() == 0))) {
			...
			try {
				...
				if(listeArticleBySearch != null) {
					...
					request.setAttribute("erreur", message);
					getServletContext().getRequestDispatcher("/WEB-INF/jsp/indexConnecte.jsp").forward(request, response);
				}
